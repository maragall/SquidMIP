...
class ProcessParameters(BaseModel, use_attribute_docstrings=True):
    ...
    """Subtract a background level before writing (IMA-224). Give the percentile of image pixels
    to treat as background, e.g. 10. Omitted = no subtraction, byte-identical to before.

    This needs its own flag rather than being a --projector value: a correction is not a
    z-reduction, so it cannot be a projector-table entry (that would break the Z=1 output
    contract), and it carries a per-run parameter a name-keyed table cannot hold."""
    ...
    def _valid_background(cls, v):
        # Validate UP FRONT, for the same reason as _known_projector below: a bad value must fail
        # before write_plate creates an empty plate skeleton on disk.
        if v is not None and not 0.0 <= v <= 100.0:
            raise ValueError(f"background must be a percentile in [0, 100], got {v}")
        return v
...
def run(params: ProcessParameters) -> dict:
    ...
    # Background subtraction (IMA-224): estimate ONE level for the run, then decorate the reducer.
    # AFTER is safe for 'mip' only — max commutes exactly with a z-invariant subtraction, at 1/Nz
    # the cost. Any other reducer gets BEFORE, which is always correct.
    ...
    if params.background is not None:
        ...
        bg_level = estimate_background(reader, params.background)
        side = AFTER if params.projector == "mip" else BEFORE
        # Distinct folder per correction: bgsub is destructive, so two percentiles must not
        # overwrite each other's plate.
        out_dir = out_parent / f"{name}.bgsub{params.background:g}.hcs"
        logger.info("background: subtracting %.6g (p%g, applied %s the z-reduction) -> %s",
                    bg_level, params.background, side, out_dir)
        projector = with_correction(_resolve_projector(params.projector),
                                    background_corrector(bg_level), side)
