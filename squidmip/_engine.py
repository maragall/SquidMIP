...
def _resolve_projector(name) -> Projector:
    ...
    Also accepts an already-built ``Projector`` callable and passes it straight through. That is
    what lets a *decorated* reducer (IMA-224 background subtraction, via
...
def project_plate(
    ...
    *,
    ...
    projector:
        A projector name from the table (default ``"mip"``), or a ``Projector`` callable. See
        :func:`add_projector`. A callable is how a per-run *correction* (IMA-224 background
        subtraction) reaches the reduce seam — it carries a parameter a name cannot.
