...
---------------------------
``projection.py`` writes ``out[t, c, 0] = reduce(planes)`` — Z is pinned to size 1, and
``_output.write_plate``, ``_montage`` and ``_viewer`` all depend on it. A *correction* is
...
WHY ONE SCALAR FOR THE WHOLE RUN
--------------------------------
The background is estimated ONCE, not per well and not per plane. A per-plane estimate varies
with focus and cell density, so a confluent field would get a larger subtraction than a sparse
one. Under a max-projection that changes *which z wins* per pixel and makes intensities
non-comparable across wells — which is the entire point of a high-content screen.
...
2. **float64 promotion.** ``np.percentile`` returns a float64 scalar, and
   ...
   breaks the native-dtype output contract. Hence the explicit cast to the plane's own dtype.
"""
...
#: Planes sampled per run when estimating the background. Bounded so the estimate costs O(1)
#: in plate size rather than growing with the number of wells.
BACKGROUND_SAMPLE_PLANES = 8

#: Stride used when sampling pixels *within* a plane for the percentile. A low percentile of a
#: strided subsample is statistically indistinguishable from the full-frame value at a fraction
#: of the cost. Implementation constant, deliberately not a second user-facing knob.
...
def subtract_background(plane: Plane, background) -> Plane:
    ...
    being taken away). Equivalent to ``clip(plane - background, 0, None)`` but without widening
    to a signed type, which keeps the bounded-memory contract of the reduce path.
    ...
    background:
        Scalar background level, or ``None`` for a no-op. Cast to *plane*'s dtype before use —
        passing the raw float64 from ``np.percentile`` would otherwise promote the result.
...
def _as_dtype(value, dtype):
    ...
    if dt.kind in "ui":
        ...
        return dt.type(min(max(v, int(info.min)), int(info.max)))
...
def with_correction(reduce: Callable[[Iterable[Plane]], Plane],
    ...
    (``Iterable[plane] -> plane``), so it drops into ``project_well(reduce=...)`` and the plate
    ...
    corr:
        A 1-plane-in/1-plane-out callable, or ``None`` for an identity pass-through (so callers
        can wire this unconditionally and let "no correction" be the null case).
    side:
...
def estimate_background(reader,
                        ...
                        *,
                        ...
                        channel=None) -> float:
    """Estimate ONE background level for the whole run from a bounded sample of raw planes.

    One scalar for the run, not one per well and not one per plane — see the module docstring
    for why per-plane estimation breaks cross-well comparability.

    Sampling is bounded (*sample_planes*) and deterministic (first wells/z in reader order), so
    cost does not grow with plate size and two runs of the same acquisition agree.
    ...
    channel:
        Channel *name* to sample (reader planes are keyed by name, not index), or ``None`` for
        the first channel in ``metadata["channels"]``.

    Returns
    -------
    float
        The estimated background level. ``0.0`` if nothing could be sampled — an empty or
        unreadable acquisition degrades to "no correction" rather than aborting a plate run.
...
def _sample_planes(reader, limit: int, channel) -> Iterator[Plane]:
    ...
    ``reader.metadata`` is a **dict** and planes are keyed by channel *name* (the reader builds
    its index key with ``str(channel)``), so both are looked up rather than assumed.
    """
...
def _iter_wells(meta, select_fovs) -> Iterator[tuple]:
