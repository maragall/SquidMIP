...
The traps this file exists to pin:
  ...
  2. float64 promotion — np.percentile returns float64, and np.maximum(uint16, float64) promotes
     the RESULT, which breaks the native-dtype output contract.
"""
...
def test_background_at_or_above_max_yields_all_zeros():
    plane = np.array([[10, 200]], dtype=np.uint16)
    np.testing.assert_array_equal(subtract_background(plane, 200), np.zeros_like(plane))
...
def test_write_provenance_records_the_level(tmp_path):
    path = write_provenance(tmp_path / "out.hcs",
                            {"background_percentile": 10, "background_level": 42})
...
def test_plate_run_is_byte_identical_when_correction_is_off(squid_dataset):
...
def test_plate_run_with_background_subtracts_and_keeps_dtype(squid_dataset):
    ...
    path, _ = squid_dataset
    plain = {(r, f): im for r, f, im in project_plate(open_reader(str(path)), projector="mip")}
    corrected = with_correction(project, background_corrector(50), AFTER)
    got = {(r, f): im for r, f, im in project_plate(open_reader(str(path)), projector=corrected)}
    for k in plain:
